...
class P1StatisticsExercise {
    ...
    private static final class Statistics {
        ...
        Snapshot snapshot() {
            ...
            lock.lock();
            Snapshot snapshot = new Snapshot(count, sum, min, max);
            lock.unlock();
            return snapshot;
        }
    }
...
class P1DiningPhilosophersHomework {
    ...
    //ОТВЕТ НА 1 ВОПРОС:Если существует полный порядок ресурсов и все потоки захватывают ресурсы строго в этом порядке,
    // взаимную блокировку организовать нельзя.(
    // (при условии, что все участники действительно соблюдают этот порядок)
    /*
    //ОТВЕТ НА 2 ВОПРОС:если ресурсы освобождать в том же порядке, в котором их брали,
    deadlock всё равно не появится, если правило общего порядка захвата ресурсов продолжает соблюдаться.
     * Вывод, означающий, что задача, скорее всего, решена правильно:
